...
Профили GPU/CPU различаются: GPU — `EMBED_BATCH_SIZE_LOCAL=256`
(видеопамяти хватает, подбиралось вверх до OOM — перезапуск безопасен),
CPU — `EMBED_BATCH_SIZE_LOCAL=32` (большой батч на CPU только медленнее
и ест RAM). Переключатель режима применяет профиль автоматически.
